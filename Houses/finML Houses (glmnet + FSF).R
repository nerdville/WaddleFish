...
# r$setResampleType(type = "StratifiedNfold", folds =  10L, stratifiedfield = "SalePrice")
 ...
 r$setResampleType(type = "RepeatedResample", train_frac = 0.8, repeats = 5L)
#  r$setResampleType(type = "Holdout", train_frac = 0.8)
...
BM$par.set$n.trees$default=10000
...
trainer<-finLearner( Resample = r
                     , BaseLearner = BM
                     , Metrics=list(rmse ="RMSE",rmsle="RMSLE",  gini="Gini" ))
...
#All features 0.12275587


featSelect<-finFeatureSelection(Trainer=trainer, Method="SFS" , MetricMaximise=F)

featSelect$RepeatedSFS(Repeats = 5)

featSelect$features.all


#0.111445096 Validation ERROR 10 repeated
#PLB 0.12315
...
write.csv(testPred, file = "Output_SFSSimpleLM.csv", row.names = F)
...
# 
 ...
 BM$setParameter(parameterName = , value = )
 optTask$opt.Results
 
 for(p in names(optTask$opt.Results$Best_Par)){
   
  BM$setParameter(parameterName = p, value = optTask$opt.Results$Best_Par[[p]])
 }
